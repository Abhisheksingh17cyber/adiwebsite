...
const Hyperspeed = ({ effectOptions = {} }: HyperspeedProps) => {
  ...
  useEffect(() => {
    ...
    class CarLights {
      ...
      init() {
        ...
        for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
          ...
          aOffset.push(laneX - carWidth / 2, offsetY, laneX + carWidth / 2, offsetY, offsetZ);
    ...
    class App {
      ...
      initPasses() {
        ...
        this.bloomPass = new EffectPass(
          this.camera,
          new BloomEffect({ luminanceThreshold: 0.2, luminanceSmoothing: 0, resolutionScale: 1 } as any),
        );
        const smaaPass = new EffectPass(
          this.camera,
          new SMAAEffect({ preset: SMAAPreset.MEDIUM } as any),
        );
    ...
    (merged as any).distortion = distortions[(merged as any).distortion] ?? distortions['turbulentDistortion'];

    const myApp = new App(container, merged);
